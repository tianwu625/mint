...
for i in $(seq 1 3)
do
	git clone --depth 1 -b 1.31.37 https://github.com/boto/botocore && break || (ping 192.168.16.7 -c 2 && echo "retry....")
done
	(cd botocore &&
...
for i in $(seq 1 3)
do
git clone --depth 1 -b 1.29.37 https://github.com/aws/aws-cli && break || echo "retry...."
done
	(cd aws-cli &&
	python3 -m pip install .) ||
